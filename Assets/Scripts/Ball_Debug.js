...
static public var S : Ball_Debug; // Singleton
...
// Game Over variables
private var _gameOverBool : boolean = false;
public var _gameOverFloat : float = 0.75f;
...
function Update () 
{
	...
	{
		_goldenGT.text = "Golden Ball Charged Up | Press the 'C' key";
	}
...
function GameOver()
{
	if (_gameOverBool == true)
	{
		_gameOverFloat -= Time.deltaTime;
	}
	if (_gameOverFloat <= 0.0f)
	{
		// Reset after Game Over
...
function GoldenBall ()
{	
	if (_specialCounter >= 5 && Input.GetKeyDown(KeyCode.C))
	{
		_specialCounter = 0;
		_theGoldenBall = true;
...
function AirGame ()
{
	if (Player.S.underBall == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space))
	{
...
function Kick ()
{
	...
	// Triple Perfect Push
	if (tppBool == true && Input.GetKeyDown(KeyCode.Space) && 
	...
	// Special Bounce
	if (_theGoldenBall == true && Input.GetKeyDown(KeyCode.Space) && 
		(perfect == true || great == true || ok == true) && 
		buttonTimer <= 0.0f && Player.S.grounded == true)
	{
		GetComponent.<Rigidbody>().AddForce(new Vector2(0, specialBounce));
	...
	// Floor Game
	if (slideL == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space) && 
		buttonTimer <= 0.0f)
	{
		Player.S.GetComponent.<Rigidbody>().AddForce(new Vector2(-Player.S.slideForce, 0));
		// Debug.Log("Slide!");

		//SlideTackle.S.ButtonSpawn(); // SlideTackle QTE
		SlideTackleQTE.S.ButtonOn(); // SlideTackle QTE
	...
	if (slideR == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space) && 
		buttonTimer <= 0.0f)
	{
		Player.S.GetComponent.<Rigidbody>().AddForce(new Vector2(Player.S.slideForce, 0));
		// Debug.Log("Slide!");
...
function BallJuggle ()
{
	...
	// Perfect
	if (perfect == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space) && 
		buttonTimer <= 0.0f)
	{
	...
	// Great
	else if (great == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space) && 
		buttonTimer <= 0.0f)
	{
	...
	// OK
	else if (ok == true && Player.S.grounded == true && Input.GetKeyDown(KeyCode.Space) && 
		buttonTimer <= 0.0f)
	{
		GetComponent.<Rigidbody>().AddForce(new Vector2(Random.Range(-okAngle, okAngle), okBounce));
